...
class Contacts(models.Model):
    ...
    def formatted_phone_number(self):
        if len(self.phone_number) == 12 and self.phone_number.startswith("+996"):
            return f"{self.phone_number[:4]} {self.phone_number[4:7]} {self.phone_number[7:10]} {self.phone_number[10:]}"
        return self.phone_number

    formatted_phone_number.short_description = 'Номер телефона (форматированный)'

    def __str__(self):
        return f'{self.job} {self.formatted_phone_number()} {self.mail} {self.address}'
